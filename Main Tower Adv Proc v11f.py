...
import FLUX_nc_fetch_v4 as Fetch_Flux
...
import IncomingSW_gapfillv19 as Shortwave_incoming
import OutgoingSW_gapfillv16a as Shortwave_outgoing
import IncomingLW_gapfillv21 as Longwave_incoming
import OutgoingLW_gapfillv13 as Longwave_outgoing
import Net_radiation_v1 as Net_radiation
import Light_response_curve_v25b as Fre_Lasslop

import FFNET_v5a as ANN
import ustar_filtering_Reichsteinv2 as ustar
...
def main():
	...
	if cf['Options']['CABLEdata_for_rainfallgap_too']=='Yes': 
	    ...
	    cable_rain=False
	New_combined=Rainstuff.Rain_gapfill(Rain_label_variable_to_fill,New_combined,myBaseforResults,Site_ID,bestAWS_ID,cable_rain,FluxFreq,Tower_Lat,Tower_Long)
	#Check dataframe for duplicates, set freq as required and pad as necessary and sort.  Call function
	New_combined = dataframe_check(New_combined, FluxFreq)  
	#to_pickle new data
	New_combined.to_pickle(DFfilename) 
    ...
    #Now Perform the ANN gap filling.  Call the routine. It is coded so that it can accept multiple inputs and outputs
    ...
    if cf['Options']['Ustar_filter']=='Yes':   
	...
	New_combined=Fre_ANN.Fre_ANN_gapfill(myBaseforResults,New_combined,Site_ID,list_in,list_out,iterations,Tower_Lat,Tower_Long,frequency,evening,min_threshold,max_threshold)
	#Check dataframe for duplicates, set freq as required and pad as necessary and sort.  Call function
	New_combined = dataframe_check(New_combined, FluxFreq)  
	#Save new data
	New_combined.to_pickle(DFfilename) 
	...
	New_combined= pd.read_pickle(DFfilename) 
	#Call GPP routine
	New_combined=GPP.GPP_calculate(myBaseforResults,New_combined,Site_ID)
	#Check dataframe for duplicates, set freq as required and pad as necessary and sort.  Call function
	New_combined = dataframe_check(New_combined, FluxFreq)  	
	#Save new data
	New_combined.to_pickle(DFfilename) 
	...
	Fingerprints.fingerprint_plots(myBaseforResults,New_combined,Site_ID,CFname)
	...
	REddyProc.Output_files(New_combined,myBaseforResults,Site_ID)  

    if cf['Options']['Output_yearly_ARM_files']=='Yes':      
	#Here do diagnostics from gap filling
	New_combined= pd.read_pickle(DFfilename) 
	#Will produce yearly files for ARM
	ARMProc.Output_files(New_combined,myBaseforResults,Site_ID)  

    if cf['Options']['Output_yearly_WAVES_files']=='Yes':      
	#Here do diagnostics from gap filling
	New_combined= pd.read_pickle(DFfilename) 
	#Will produce yearly files for WAVES model
	WAVESProc.Output_files(New_combined,myBaseforResults,Site_ID)  
