...
def Doplots_monthly(CABLEandTOWER_DF,Site_ID,mypathforResults):   
    ...
    plot_list=['SWdown_CABLE','LWdown_CABLE','Fsd','Fld','Rnet_CABLE','Qh_CABLE','Qg_CABLE','Qle_CABLE','Qs_CABLE','GPP_CABLE','Fc']
...
def fetch_CABLE(FLUXDataframe,myBaseforResults,CABLEfilename,Site_ID,latitude,longitude):
    ...
    #then sort and check for duplicates
    theDataFrame.sort_index(inplace=True)
   
    theDataFrame["index"] = theDataFrame.index
    theDataFrame.drop_duplicates(cols='index', inplace=True)
    del theDataFrame["index"]

    print "Length of series after duplicates removed ",len(theDataFrame)   
	...
	dailyCABLE_DF_grouped=theDataFrame.groupby([lambda x: x.year, lambda x: x.month,lambda x: x.day])
	for groupindex, group in dailyCABLE_DF_grouped:
