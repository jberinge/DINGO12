...
import FFNET_Fre_v5a as Fre_ANN
import GPP_calc_v1c as GPP
...
def main():
	...
	if Site_ID == 'Warra':
	     New_combined['Ts']=np.nan
	     New_combined['Sws']=np.nan
	...
	New_combined= pd.read_pickle(DFfilename) 
	#Call GPP routine
	New_combined=GPP.GPP_calculate(myBaseforResults,New_combined,Site_ID,versionID)
	#Check dataframe for duplicates, set freq as required and pad as necessary and sort.  Call function
	New_combined = dataframe_check(New_combined, FluxFreq)  	
	#Save new data
	New_combined.to_pickle(DFfilename) 
