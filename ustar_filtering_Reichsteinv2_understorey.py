...
def ustar_filter(New_combined,myBaseforResults,Site_ID):
    ...
    for index,group in New_combined[['Ts_Con','ustar']].groupby([lambda x: x.year,lambda x: x.month]):
        ...
        ustar_DF=New_combined[['Ts_Con','Fc','ustar','Fn']].dropna(axis=0,how='any')
        
        #Choose date and only nightime where Fsd is less than 10 Wm-2
        ustar_DF=ustar_DF[startdate:enddate][ustar_DF['Fn']<-10]
        ...
        if len(ustar_DF)>400:
            ...
            try:
                results=temp_grouped['Fc','ustar'].apply(dotempbin, args=(DFyear,DFmonth)).reset_index(drop=True)
